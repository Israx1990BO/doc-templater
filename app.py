...
try:
    ...
    if FROZEN:
        import pytesseract
        _tess_bin = os.path.join(BUNDLE_DIR, 'tesseract', 'tesseract')
    ...
    def generate():
        ...
        for original_name, filename, input_path in saved_files:
            ...
            try:
                ...
                # Build output filename with prefix/suffix
                ...
                import traceback
                traceback.print_exc()
                yield json.dumps({'msg': 'Error en {}: {}'.format(original_name, str(e)), 'type': 'error'}) + '\n'
        ...
        yield json.dumps({'type': 'complete', 'results': results, 'output_folder': target_folder, 'stats': stats}) + '\n'
